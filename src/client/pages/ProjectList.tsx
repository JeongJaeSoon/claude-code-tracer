...
export function ProjectList({ onSelectSession }: ProjectListProps) {
  ...
  async function fetchProjectSessions(projectName: string) {
    ...
    try {
      ...
      setProjectSessions((prev) => new Map(prev).set(projectName, data.sessions || []));
    } catch (error) {
                ...
                >
                  ...
                  <div className="project-stats">
                    <div className="project-stat">
                      <span className="stat-number">{project.sessionCount}</span>
                      <span className="stat-unit">sessions</span>
                    </div>
                    <div className="project-stat">
                      <span className="stat-number">{formatDuration(project.totalDurationMs)}</span>
                      <span className="stat-unit">total</span>
                    </div>
                    <div className="project-stat">
                      <span className="stat-number">{formatTokens(project.totalInputTokens + project.totalOutputTokens)}</span>
                      <span className="stat-unit">tokens</span>
                    </div>
                    <div className="project-stat">
                      <span className="stat-number">{project.totalToolCalls}</span>
                  ...
                  <div className="expand-icon">
                    <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={expandedProject === project.projectName ? "M19 9l-7 7-7-7" : "M9 5l7 7-7 7"}/>
                    </svg>
                  </div>
                </div>
                  ...
                  <div className="project-sessions">
                    {loadingSessions === project.projectName ? (
                      ...
                      <div className="sessions-table">
                        ...
                        {(projectSessions.get(project.projectName) || []).map((session) => (
                          <div
                            key={session.id}
                          ...
                          >
                            ...
                            <div className="session-agents">{session.subAgentCount > 0 ? `+${session.subAgentCount}` : "-"}</div>
                            <div>
                              <span className={`status-badge ${session.status}`}>
                                <span className="status-dot"></span>
                                {session.status === "completed" ? "Done" : session.status === "running" ? "Running" : "Error"}
                              </span>
                            </div>
                          </div>
                        ))}
