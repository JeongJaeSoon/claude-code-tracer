...
export async function parseJSONL(content: string): Promise<ParseResult> {
	...
	const toolUseMap = new Map<
		string,
		{ name: string; input: Record<string, unknown>; startTime: number }
	>();
	...
	for (const msg of validMessages) {
		...
		if (msg.type === "user" || msg.type === "assistant") {
			...
			if (Array.isArray(msg.message?.content)) {
				...
				const hasTextContent = msg.message.content.some(
					(b) => b.type === "text",
				);
				const hasThinkingContent = msg.message.content.some(
					(b) => b.type === "thinking",
				);
				...
				if (hasOnlyToolResult && !hasTextContent && !hasThinkingContent) {
					...
					thinking = msg.message.content
						...
						.map(
							(b) =>
								(b as { type: "thinking"; thinking?: string }).thinking || "",
						)
						.join("\n");
				}
			} else {
