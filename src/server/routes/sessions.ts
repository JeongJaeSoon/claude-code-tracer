...
sessionsRoutes.get("/", async (c) => {
  try {
    ...
    const conditions: (ReturnType<typeof eq> | ReturnType<typeof like> | ReturnType<typeof gte> | ReturnType<typeof lte>)[] = [];
    ...
    if (status && status !== "all") {
      conditions.push(eq(sessions.status, status as "completed" | "running" | "error"));
    }
